...
def build_agents(model: str, api_key: str, config: AppConfig) -> AgentBundle:
    ...
    return AgentBundle(
        ...
        scout_web=build_agent(model, api_key, "web_search", 0.2, []),
        scout_local=build_agent(model, api_key, "local_rag", 0.2, []),
        evidence_judge=build_agent(model, api_key, "deep_dive", 0.2, []),
        analyst=build_agent(model, api_key, "analyze", 0.2, []),
        fact_checker=build_agent(model, api_key, "fact_check", 0.0, []),
        direct_responder=build_agent(model, api_key, "direct_answer", 0.2, []),
        writer=build_agent(model, api_key, "write", 0.1, []),
    )
