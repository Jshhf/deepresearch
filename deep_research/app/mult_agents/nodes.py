...
LLM_NODE_TIMEOUT_SECONDS = int(os.getenv("LLM_NODE_TIMEOUT_SECONDS", "120"))
...
def _invoke_agent_with_retry(agent, messages: list, node: str):
    """带节点级超时和重试的 LLM 调用，避免 DashScope 长连接拖死工作流。"""
    ...
    for attempt in range(1, LLM_MAX_RETRIES + 1):
        ...
        def worker():
            try:
                result_queue.put(("ok", agent.invoke({"messages": messages})))
        ...
        thread.start()
        try:
            status, value = result_queue.get(timeout=LLM_NODE_TIMEOUT_SECONDS)
            if status == "ok":
                return value
            last_error = value
        except queue.Empty:
            last_error = TimeoutError(f"LLM 调用超时（>{LLM_NODE_TIMEOUT_SECONDS}s）")
            logger.warning(
                "%s 第 %s/%s 次调用超时，准备重试",
...
def _invoke_json_agent(state: ResearchState, prompt: str, agent, agent_name: str, node: str, fallback: dict) -> tuple[dict, str, list]:
    ...
    try:
        ...
        result = _invoke_agent_with_retry(agent, [human], node)
    except Exception as exc:
        logger.error(
            "%s 调用失败，使用确定性回退: %s",
            colorize(f"[{node}]", "red"),
        ...
        return fallback, json.dumps(fallback, ensure_ascii=False), [human]
...
def _invoke_text_agent(state: ResearchState, prompt: str, agent, agent_name: str, node: str) -> tuple[str, list]:
    human = HumanMessage(content=with_memory_context(state, prompt))
    result = _invoke_agent_with_retry(agent, [human], node)
    content = _last_content(result).strip()
    emit(node, content)
    return content, [human, result["messages"][-1]]
...
def _derive_search_plan(outline: list[dict], sub_questions: list[str], _research_questions: list[str], query: str) -> list[dict]:
    ...
    for index, sub_question in enumerate(sub_questions or [], 1):
        text = str(sub_question).strip()
        if not text or text == query or any(item.get("query") == text for item in plan):
            continue
        if _is_query_grounded(text, query):
            plan.append(
                {
                    "section_id": f"subq_{index}",
                    "query": text,
                    "source_preference": "hybrid",
                    "reason": f"来自 Planner 子问题 {index}",
                }
            )
...
def _enrich_evidence_from_raw(evidence: list[dict], raw_records: list[dict]) -> list[dict]:
    """强制以原始检索记录为准，覆盖 LLM 可能编造的 url/title/snippet 等字段。"""
    ...
    for ev in evidence:
        ...
        for field in ("url", "domain", "title", "snippet", "source_type", "published_at"):
            value = raw.get(field)
            if isinstance(value, str) and value.strip():
                item[field] = value
            elif value is not None:
                item[field] = value
        if raw.get("doc_id") and not item.get("doc_id"):
            item["doc_id"] = raw["doc_id"]
...
def _score_evidence(record: dict) -> tuple[float, str]:
    ...
    if any(word in domain for word in [
        "github", "arxiv", "wikipedia", "huggingface", "openai", "anthropic",
        "microsoft", "google", "langchain", "crewai", "pypi", "docs",
    ]):
        return 0.76, "官方或技术文档域名"
    if domain:
...
def _is_authoritative_domain(domain: str) -> bool:
    ...
    return any(
        ...
        for word in [
            "github", "arxiv", "wikipedia", "huggingface", "openai", "anthropic",
            "microsoft", "google", "langchain", "crewai", "pypi", "docs",
            "news", "finance", "reuters", "bloomberg", "people", "xinhuanet",
        ]
    )
...
def _is_low_quality_domain(domain: str) -> bool:
    ...
    return any(
        word in value
        for word in [
            "csdn", "juejin", "cnblogs", "zhihu", "bilibili", "douyin",
            "sohu", "163.com", "yidianzixun", "toutiao", "php", "game",
            "xiaohongshu", "weibo", "baijiahao", "9game", "17173",
        ]
    )


def _should_cite_record(record: dict) -> bool:
    domain = str(record.get("domain", "")).lower()
    if record.get("source_type") == "local":
        return True
    return not _is_low_quality_domain(domain)
...
def _published_year(record: dict) -> int | None:
...
def _filter_web_records_by_time(records: list[dict], query: str) -> tuple[list[dict], int]:
    ...
    dropped = 0
    for record in records:
        published_year = _published_year(record)
        if published_year is not None and published_year > target_year:
            dropped += 1
            continue
        kept.append(record)
    return kept, dropped
...
def _format_evidence_pool_for_writer(evidence_pool: list[dict]) -> str:
    """把可写入证据池压缩为 writer 可直接引用的原文证据。"""
    if not evidence_pool:
        return "[]"
    lines = []
    for item in evidence_pool[:40]:
        if not isinstance(item, dict):
            continue
        lines.append(
            json.dumps(
                {
                    ...
                    "title": item.get("title") or item.get("source_label"),
                    "url": item.get("url", ""),
...
def _strip_unapproved_links(content: str, valid_locators: list[str]) -> str:
    """正文中只保留指向真实来源的 Markdown 链接，其余链接退化为纯文本。"""
    valid = {str(locator).strip().rstrip("/") for locator in valid_locators if str(locator).strip()}

    def replace_link(match):
        label = match.group(1).strip()
        url = match.group(2).strip()
        if url.rstrip("/") in valid:
            return f"[{label}]({url})"
        return label
...
def direct_answer_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    try:
        content, messages = _invoke_text_agent(
            state,
            prompt,
            agent,
            agent_name,
        ...
        logger.error("%s 直接回答失败，返回降级文案: %s", colorize("[direct_answer]", "red"), exc)
        content = "暂时无法完成回答，请稍后重试。"
        messages = [HumanMessage(content=with_memory_context(state, prompt))]
    return {
        "intent": "direct",
        "final": content,
        "draft": content,
...
def web_search_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    raw_records, dropped_time_mismatch = _filter_web_records_by_time(raw_records, state["query"])
    if dropped_time_mismatch:
        logger.warning(
            "[web_search_node] 过滤时效错位记录 | dropped=%s | query=%s",
            dropped_time_mismatch,
            state["query"],
        )
...
def local_rag_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    for query_index, item in enumerate(queries, 1):
        records = search_knowledge_base_records(str(item.get("query", "")), limit=6)
        records = _assign_source_ids(records, f"{prefix}_{query_index}")
        for record in records:
            record["section_id"] = item.get("section_id")
            record["search_query"] = item.get("query")
...
def deep_dive_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    # 证据裁判返回的 url/title/snippet 也一律以原始检索记录为准
    evidence_pool = _enrich_evidence_from_raw(evidence_pool, raw_evidence)
...
def write_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    ...
    prompt = (
        ...
        "【可引用原文证据 (Evidence)】：\n"
        f"{_format_evidence_pool_for_writer(state.get('evidence_pool', []))}\n\n"
        "【可用来源索引 (source_index)】：\n"
        ...
        "每个事实句必须至少绑定一个 Evidence 中的 source_id；没有对应原文支持的句子写“待核实”，不得补写数字、日期、机构名或链接。"
        "如果证据只支持定性结论，就只写定性结论，不要扩写成具体数字。"
        "正文中不得自行生成任何 http(s) 链接，系统会在结尾自动拼接参考资料；"
    ...
    try:
        ...
        logger.error("%s 写作调用失败，使用基础报告回退: %s", colorize("[write]", "red"), exc)
        content = _render_fallback_report(state)
        messages = [HumanMessage(content=with_memory_context(state, prompt))]
    ...
    # 正文中的 Markdown 链接必须来自真实来源，否则退化为纯文本
    valid_locators = [str(item.get("locator", "")) for item in state.get("source_index", [])]
    content = _strip_unapproved_links(content, valid_locators)
...
def _fallback_fact_check(state: ResearchState) -> dict:
    return {
        "final_report": state.get("draft") or state.get("final", ""),
        "flagged_claims": [],
    }


def fact_check_node(state: ResearchState, agent, agent_name: str) -> ResearchState:
    logger.info("%s 开始 | agent=%s", colorize("[fact_check]", "cyan"), colorize(agent_name, "magenta"))
    draft = str(state.get("draft") or state.get("final", "")).strip()
    if not draft:
        logger.info("%s 无待校验正文，跳过", colorize("[fact_check]", "yellow"))
        return {}

    valid_source_ids = [str(item.get("source_id", "")).strip() for item in state.get("source_index", []) if item.get("source_id")]
    valid_source_ids = [item for item in valid_source_ids if item][:80]
    valid_source_ids_set = set(valid_source_ids)
    fallback = _fallback_fact_check(state)

    prompt = (
        "请对以下研报做写后事实校验，并只输出 JSON。\n"
        f"核心问题：{state['query']}\n"
        "【可用原文证据 (Evidence)】：\n"
        f"{_format_evidence_pool_for_writer(state.get('evidence_pool', []))}\n\n"
        "【合法引用ID列表】：\n"
        ...
        "【研报正文】：\n"
        f"{draft[:16000]}\n\n"
        "请返回 {\"final_report\":\"修正后的正文\",\"flagged_claims\":[{\"text\":\"...\",\"reason\":\"...\"}]}。"
    )
    ...
    final_report, _ = _validate_and_fix_citations(final_report, valid_source_ids_set)
    valid_locators = [str(item.get("locator", "")) for item in state.get("source_index", [])]
    final_report = _strip_unapproved_links(final_report, valid_locators)
    final_report = _ensure_reference_section(final_report, state)

    flagged_claims = payload.get("flagged_claims")
    if not isinstance(flagged_claims, list):
        flagged_claims = []

    emit("fact_check", final_report)
    return {
        "draft": final_report,
        "final": final_report,
        "fact_check": content,
